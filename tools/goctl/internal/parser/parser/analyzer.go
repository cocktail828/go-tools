...
func (a *Analyzer) fillInfo() {
	...
	for key, value := range properties {
		titleKey := stringx.Title(strings.TrimSuffix(key, ":"))
		infoKeyValue[titleKey] = value
	}
	a.spec.Info.Properties = map[string]string{
		"Title":   infoKeyValue[infoTitleKey],
		"Desc":    infoKeyValue[infoDescKey],
		"Version": infoKeyValue[infoVersionKey],
		"Author":  infoKeyValue[infoAuthorKey],
		"Email":   infoKeyValue[infoEmailKey],
	}
}
...
func Parse(filename string, src any) (*spec.ApiSpec, error) {
	p := New(filename, src)
	ast := p.Parse()
...
var kind = map[string]parser.Type{
