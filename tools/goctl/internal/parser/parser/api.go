...
func (api *API) checkTypeDeclareContext() error {
	var typeMap = map[string]parser.Type{}
	for _, v := range api.TypeStmt {
		switch tp := v.(type) {
		case *ast.TypeLiteralStmt:
			typeMap[tp.Expr.Name.Token.Text] = parser.PlaceHolder
		case *ast.TypeGroupStmt:
			for _, v := range tp.ExprList {
				typeMap[v.Name.Token.Text] = parser.PlaceHolder
			}
		}
	}
...
func (api *API) checkTypeContext(declareContext map[string]parser.Type) error {
...
func (api *API) checkTypeExprContext(declareContext map[string]parser.Type, tp ast.DataType) error {
	switch val := tp.(type) {
