...
func (g *Generator) GenServer(ctx DirContext, proto parser.Proto, c *ZRpcContext) error {
	if !c.Multiple {
		return g.genServerInCompatibility(ctx, proto, c)
	}

	return g.genServerGroup(ctx, proto)
}

func (g *Generator) genServerGroup(ctx DirContext, proto parser.Proto) error {
	dir := ctx.GetServer()
	for _, service := range proto.Service {
		...
		serverFile = filepath.Join(dir.Filename, serverDir, stringx.ToSnake(service.Name+"_server")+".go")
		...
		if err = util.With("server").GoFmt(true).Parse(text).SaveTo(map[string]any{
			...
			"server":    stringx.ToCamel(service.Name),
...
func (g *Generator) genServerInCompatibility(ctx DirContext, proto parser.Proto, c *ZRpcContext) error {
	dir := ctx.GetServer()
	...
	serverFile := filepath.Join(dir.Filename, stringx.ToSnake(service.Name+"_server")+".go")
...
func (g *Generator) genFunctions(goPackage string, service parser.Service, multiple bool) ([]string, error) {
	...
	for _, rpc := range service.RPC {
		...
		if !multiple {
			...
			logicPkg = strings.ToLower(stringx.ToCamel(nameJoin))
			logicName = fmt.Sprintf("%sLogic", stringx.ToCamel(rpc.Name))
		}
