...
func (g *Generator) GenCall(ctx DirContext, proto parser.Proto,
	c *ZRpcContext) error {
	if !c.Multiple {
		return g.genCallInCompatibility(ctx, proto)
	}

	return g.genCallGroup(ctx, proto)
}

func (g *Generator) genCallGroup(ctx DirContext, proto parser.Proto) error {
	dir := ctx.GetCall()
	...
	for _, service := range proto.Service {
		...
		callFilename := stringx.ToSnake(service.Name)
		...
		serviceName := stringx.ToCamel(service.Name)
		...
		if hasSameNameBetweenMessageAndService {
			serviceName = stringx.ToCamel(service.Name + "_zrpc_client")
		}
...
func (g *Generator) genCallInCompatibility(ctx DirContext, proto parser.Proto) error {
	dir := ctx.GetCall()
