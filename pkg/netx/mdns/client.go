...
func (s *ServiceEntry) complete() bool {
	return (s.AddrV4 != nil || s.AddrV6.IP != nil) && s.Port != 0 && s.hasTXT
}
...
type client struct {
	...
	closed atomic.Bool
	// closedCh is closed by Close to unblock QueryContext's cancellation
	// watcher and the recv goroutines during shutdown.
...
func newClient(v4 bool, v6 bool) (*client, error) {
	...
	if v4 {
		...
		if err != nil {
			mdnsLogger.Printf("[ERR] mdns: Failed to bind to udp4 port: %v", err)
		}
	}
	if v6 {
		uconn6, err = net.ListenUDP("udp6", &net.UDPAddr{IP: net.IPv6zero, Port: 0})
		if err != nil {
			mdnsLogger.Printf("[ERR] mdns: Failed to bind to udp6 port: %v", err)
		}
	}
	...
	if v4 {
		mconn4, err = net.ListenMulticastUDP("udp4", nil, v4mDNSAddr)
		if err != nil {
			mdnsLogger.Printf("[ERR] mdns: Failed to bind to udp4 port: %v", err)
		}
	}
	if v6 {
		mconn6, err = net.ListenMulticastUDP("udp6", nil, v6mDNSAddr)
		if err != nil {
			mdnsLogger.Printf("[ERR] mdns: Failed to bind to udp6 port: %v", err)
		}
	}
	...
	if uconn4 == nil || mconn4 == nil {
		mdnsLogger.Printf("[INFO] mdns: Failed to listen to both unicast and multicast on IPv4")
		uconn4 = nil
		mconn4 = nil
		v4 = false
	}
	if uconn6 == nil || mconn6 == nil {
		mdnsLogger.Printf("[INFO] mdns: Failed to listen to both unicast and multicast on IPv6")
		uconn6 = nil
		mconn6 = nil
		v6 = false
	}
	...
	for {
		...
		case resp := <-msgCh:
			...
			for _, answer := range append(resp.msg.Answer, resp.msg.Extra...) {
				...
				case *dns.PTR:
					// Only accept PTR records for the service we asked about.
					// Other service types may be advertising on the same
					// multicast group; their PTRs must not create entries here.
					// (The follow-up record types below are owned by
					// instance/host names rather than the service name, so they
					// are matched by name via ensureName instead.)
			...
			if inp.complete() {
				...
				m.RecursionDesired = false
				if err := c.sendQuery(m); err != nil {
					mdnsLogger.Printf("[ERR] mdns: Failed to query instance %s: %v", inp.Name, err)
				}
			}
		case <-finish:
			return nil
...
func (c *client) sendQuery(q *dns.Msg) error {
	...
	if c.ipv4UnicastConn != nil {
		_, err = c.ipv4UnicastConn.WriteToUDP(buf, v4mDNSAddr)
		if err != nil {
			return err
		}
	}
	if c.ipv6UnicastConn != nil {
		_, err = c.ipv6UnicastConn.WriteToUDP(buf, v6mDNSAddr)
		if err != nil {
			return err
		}
	}
...
func (c *client) recv(l *net.UDPConn, msgCh chan *msgAddr) {
	...
	for !c.closed.Load() {
		...
		if err != nil {
			mdnsLogger.Printf("[ERR] mdns: Failed to read packet: %v", err)
			continue
		}
		msg := new(dns.Msg)
