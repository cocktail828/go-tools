...
var (
	v4mDNSAddr = &net.UDPAddr{
		IP:   net.ParseIP("224.0.0.251"),
		Port: 5353,
	}
	v6mDNSAddr = &net.UDPAddr{
...
type Config struct {
	...
	// Probe, if non-nil, makes NewServer run the RFC 6762 probing phase before
	// it starts responding: it queries for the Zone's unique name and, on
	// conflict, renames the Zone until a free name is found. This requires Zone
	// to implement ProbeableZone (*MDNSService does); otherwise NewServer
	// returns an error. When nil, no probing is performed (the historical
	// behaviour).
	Probe *ProbeConfig
}
...
func NewServer(config *Config) (*Server, error) {
	...
	ipv4List, _ := net.ListenMulticastUDP("udp4", config.Iface, v4mDNSAddr)
	ipv6List, _ := net.ListenMulticastUDP("udp6", config.Iface, v6mDNSAddr)
	...
	// Optionally run the RFC 6762 probing phase before we start responding, so
	// a conflicting name is resolved (via renaming) up front. This must happen
	// before the recv goroutines start reading the zone's name.
	if config.Probe != nil {
		z, ok := config.Zone.(ProbeableZone)
		if !ok {
			return nil, fmt.Errorf("mdns: Config.Probe set but Zone %T does not implement ProbeableZone", config.Zone)
		}
		if err := Probe(context.Background(), z, config.Probe); err != nil {
			return nil, fmt.Errorf("mdns: probing failed: %w", err)
		}
	}
...
func (s *Server) recv(c *net.UDPConn) {
	...
	for !s.closed.Load() {
		...
		if err := s.parsePacket(buf[:n], from); err != nil {
			mdnsLogger.Printf("[ERR] mdns: Failed to handle query: %v", err)
		}
	}
}
...
func (s *Server) parsePacket(packet []byte, from net.Addr) error {
	...
	if err := msg.Unpack(packet); err != nil {
		mdnsLogger.Printf("[ERR] mdns: Failed to unpack packet: %v", err)
		return err
	}
...
func (s *Server) handleQuery(query *dns.Msg, from net.Addr) error {
	...
	if s.config.LogEmptyResponses && len(multicastAnswer) == 0 && len(unicastAnswer) == 0 {
		...
		mdnsLogger.Printf("no responses for query with questions: %s", strings.Join(questions, ", "))
	}
...
//
// TODO(reddaly): Sending is always directed at the querier's source address,
// which works for the common unicast-response case but is not fully RFC 6762
// compliant: multicast responses (the default when the querier does not set the
// unicast-response bit) should be sent to the mDNS group so other hosts can
// prime their caches. Doing that correctly requires binding the response to the
// right outgoing multicast interface, which this server does not track, so the
// unicast-to-querier behaviour is retained for now.
