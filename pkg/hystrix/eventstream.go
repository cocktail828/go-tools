...
import (
	...
	"github.com/cocktail828/go-tools/pkg/hystrix/rolling"
)
...
// NewStreamHandler returns a server capable of exposing dashboard statistic via HTTP.
func NewStreamHandler() *StreamHandler {
	return &StreamHandler{}
}

// StreamHandler publishes statistic for each command and each pool once a second to all connected HTTP client.
...
// Start begins watching the in-memory circuit breakers for statistic
...
func (sh *StreamHandler) loop() {
	...
	for {
		...
		case <-tick.C:
			...
			for _, cb := range circuitBreakerMap {
				sh.publishMetrics(cb)
				sh.publishThreadPools(cb)
			}
...
func (sh *StreamHandler) publishMetrics(cb *circuitBreaker) error {
	now := time.Now()
	reqCount := cb.statistic.Requests().Sum(now)
	errCount := cb.statistic.DefaultCollector().Errors().Sum(now)
	errPct := cb.statistic.ErrorPercent(now)
	...
	eventBytes, err := json.Marshal(&streamCmdMetric{
		...
		RollingCountSuccess:            uint32(cb.statistic.DefaultCollector().Successes().Sum(now)),
		RollingCountFailure:            uint32(cb.statistic.DefaultCollector().Failures().Sum(now)),
		RollingCountThreadPoolRejected: uint32(cb.statistic.DefaultCollector().Rejects().Sum(now)),
		RollingCountShortCircuited:     uint32(cb.statistic.DefaultCollector().ShortCircuits().Sum(now)),
		RollingCountTimeout:            uint32(cb.statistic.DefaultCollector().Timeouts().Sum(now)),
		RollingCountFallbackSuccess:    uint32(cb.statistic.DefaultCollector().FallbackSuccesses().Sum(now)),
		RollingCountFallbackFailure:    uint32(cb.statistic.DefaultCollector().FallbackFailures().Sum(now)),

		LatencyTotal:       generateLatencyTimings(cb.statistic.DefaultCollector().TotalDuration()),
		LatencyTotalMean:   cb.statistic.DefaultCollector().TotalDuration().Mean(),
		LatencyExecute:     generateLatencyTimings(cb.statistic.DefaultCollector().RunDuration()),
		LatencyExecuteMean: cb.statistic.DefaultCollector().RunDuration().Mean(),
		...
		CircuitBreakerForceOpen:              cb.forceOpen.Load(),
		CircuitBreakerErrorThresholdPercent:  uint32(getCircuit(cb.Name).ErrorPercentThreshold),
		CircuitBreakerSleepWindow:            uint32(getCircuit(cb.Name).SleepWindow.Seconds() * 1000),
		CircuitBreakerRequestVolumeThreshold: uint32(getCircuit(cb.Name).RequestVolumeThreshold),
	})
	if err != nil {
		return err
	}
...
func (sh *StreamHandler) publishThreadPools(cb *circuitBreaker) error {
	// now := time.Now()
	eventBytes, err := json.Marshal(&streamThreadPoolMetric{
		Type:                      "HystrixThreadPool",
		Name:                      cb.Name,
		ReportingHosts:            1,
		CurrentActiveCount:        uint32(cb.ActiveCount()),
		CurrentTaskCount:          0,
		CurrentCompletedTaskCount: 0,
		// RollingCountThreadsExecuted: uint32(cb.etrics.Executed.Sum(now)),
		// RollingMaxActiveThreads:     uint32(cb.Metrics.MaxActiveRequests.Max(now)),
		CurrentPoolSize:             uint32(cb.RequestVolumeThreshold),
		CurrentCorePoolSize:         uint32(cb.RequestVolumeThreshold),
		CurrentLargestPoolSize:      uint32(cb.RequestVolumeThreshold),
		CurrentMaximumPoolSize:      uint32(cb.RequestVolumeThreshold),
		RollingStatsWindow:          10000,
