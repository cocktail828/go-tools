...
func (u *Userinfo) IsSetPassword() bool {
	if u == nil {
		return false
	}
...
// support multiaddr
func parseHost(rawhost string) (string, error) {
	for _, host := range strings.Split(rawhost, ",") {
	...
	var err error
	if rawhost, err = unescape(rawhost, encodeHost); err != nil {
		return "", err
	}
	return rawhost, nil
}
...
// trim the leading and tailing slash from Path
func (u *URL) BarePath() string {
...
func (u *URL) Redacted() string {
	...
	ru := *u
	if has := ru.User.IsSetPassword(); has {
