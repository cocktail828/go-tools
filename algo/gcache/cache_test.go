...
func TestLoaderExpireFuncWithoutExpire(t *testing.T) {
	...
	for _, builder := range testCaches {
		...
		cache := builder.
			...
			EvictedFunc(func(key, value any) { panic(key) }).Build()
		...
		if testCounter != 1 {
			t.Errorf("[%v] testCounter(%v) != 1", cache.EvictType(), testCounter)
		}
	}
}
...
func TestDeserializeFunc(t *testing.T) {
	...
	for _, cs := range cases {
		...
		cc := New(32).
			EvictType(cs.tp).
			LoaderFunc(func(k any) (any, error) {
				return value1, nil
			}).
			DeserializeFunc(func(k, v any) (any, error) {
			...
			SerializeFunc(func(k, v any) (any, error) {
