...
// A resolverConf represents a DNS stub resolver configuration.
type resolverConf struct {
	initOnce sync.Once // guards init of resolverConf
...
var resolvConf resolverConf

func loadResolverConf() *DNSConfig {
	resolvConf.tryUpdate("/etc/resolv.conf")
	return resolvConf.dnsConfig.Load()
}

// init initializes conf and is only called via conf.initOnce.
func (conf *resolverConf) init() {
	// Set DNSConfig and lastChecked so we don't parse
	// resolv.conf twice the first time.
	conf.dnsConfig.Store(dnsReadConfig("/etc/resolv.conf"))
	conf.lastChecked = time.Now()

	// Prepare ch so that only one update of resolverConf may
	// run at once.
...
// "/etc/resolv.conf".
func (conf *resolverConf) tryUpdate(name string) {
...
func (conf *resolverConf) tryAcquireSema() bool {
	select {
	case conf.ch <- struct{}{}:
		return true
	default:
...
func (conf *resolverConf) releaseSema() {
	<-conf.ch
}
