...
func ParseBase32(b []byte) (ID, error) {
	...
	for i := range b {
		if decodeBase32Map[b[i]] == 0xFF {
			return -1, fmt.Errorf("invalid base32 character %c in input: %w", b[i], ErrInvalidBase32)
		}
...
func ParseBase58(b []byte) (ID, error) {
	...
	for i := range b {
		if decodeBase58Map[b[i]] == 0xFF {
			return -1, fmt.Errorf("invalid base58 character %c in input: %w", b[i], ErrInvalidBase58)
		}
...
func (f *ID) UnmarshalJSON(b []byte) error {
	...
	if err != nil {
		return fmt.Errorf("failed to parse JSON snowflake ID %q: %w", string(b), err)
	}
